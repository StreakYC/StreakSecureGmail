...
}).call(StreakSecureGmail);

StreakSecureGmail._ = StreakSecureGmail._underscore;(function(StreakSecureGmail){
...
})(StreakSecureGmail, window);(function(StreakSecureGmail) {
	var _ = StreakSecureGmail._;
...
(function(StreakSecureGmail) {
  ...
  _.extend(Gmail.prototype, {
    initialize: function() {
      ...
      var checkTimer = _.repeatEvery(function() {
        self.bootstrap();
    ...
    bindWatchers: function() {
      var self = this;
			// 8.21.18: We are preventing all encrpytion so that only decryption may continue going forward.
      // this._setupComposewatcher();
      //this._setupXHRWatcher();

      StreakSecureGmail.DomWatcher.watchForNewSelector(
        ".Bu .nH.if .h7",
    ...
    _setupComposewatcher: function() {
      ...
      StreakSecureGmail.DomWatcher.watchForNewSelector(
        ".Wk [role=dialog] .Hy, .dw [role=dialog] .Hy",
        function(element) {
    ...
    _setupXHRWatcher: function() {
      ...
      var win = top.document.getElementById("js_frame").contentDocument
        .defaultView;
      ...
      xmlProto.open = function(method, url, async, user, password) {
        ...
        if (url) {
          try {
            ...
            if (
              queryParameters.act &&
              queryParameters.act.length > 0 &&
              queryParameters.act[0] === "sd"
            ) {
      ...
      xmlProto.send = function(body) {
        ...
        if (
          ...
          out = xmlProto._gmail_send.apply(this, [
            self._encodeQueryParameters(queryParameters)
          ]);
        } else {
...
(function(StreakSecureGmail){
	...
	Gmail.bind('draftSaving', function(){
		// a draft is trying to be saved, let's make sure the markers are there
		for(var ii=0; ii<_secureComposeWindows.length; ii++){
			var body = _secureComposeWindows[ii].find('.Ap [contenteditable=true]');

			//is this an email we're actually trying to send?
			var sendMarker = body.find('[hspace=streakMarkerOuter]');
			if(sendMarker.length > 0){
				continue;
			}

			var secureMarker = body.find('[hspace=streakSecureMarker]');
			if(secureMarker.length === 0){
				secureMarker = document.createElement('span');
				secureMarker.setAttribute('hspace', 'streakSecureMarker');
				body.append(secureMarker);
			}
		}
	});
	...
	function setupCompose(compose){
		...
		title[0].style.display = 'inline-block';

		var titleContainer = $(title[0].parentNode);

		var secureTitle = document.createElement('div');
		secureTitle.style.color = 'white';
		secureTitle.style.display = 'inline-block';
		secureTitle.style.marginLeft = '5px';
		...
		titleContainer.append(secureTitle);
		...
		currentSend[0].removeAttribute('tabindex'); //get it out of the tab order


		/* add marker */
		var body = compose.find('.Ap [contenteditable=true]');
		var secureMarker = document.createElement('span');
		secureMarker.setAttribute('hspace', 'streakSecureMarker');
		body.append(secureMarker);


		/* prevent meta+enter */
		compose[0].addEventListener(
			'keydown',
			function(e){
	...
	function getPasswordInfo(compose, currentSend){
		...
		inner[0].innerHTML = [
			...
			'<div class="streak__extraHint">Instead of telling the recipients the password, give them a hint that only they would know</div>'
		].join("");
